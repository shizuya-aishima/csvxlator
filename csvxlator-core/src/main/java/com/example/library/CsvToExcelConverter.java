...
/**
 * CSVファイルをExcelファイルに変換するユーティリティクラスです。 このクラスは、Apache Commons CSVとApache
 * POIを使用してCSVからExcel形式への変換を行います。 変換時にはヘッダー行のスタイリングやカラム幅の自動調整などの基本的な書式設定も行います。
 */
public class CsvToExcelConverter {
  ...
  /** デフォルトのコンストラクタ。 CSV形式はRFC4180準拠、文字コードはUTF-8を使用します。 */
  ...
  public void convert(File csvFile, File excelFile) throws IOException {
    ...
    try (Reader reader =
            new BufferedReader(new InputStreamReader(new FileInputStream(csvFile), charset));
